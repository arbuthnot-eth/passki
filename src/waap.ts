...
import type { SocialProvider, AuthenticationMethod } from '@human.tech/waap-interface-core';
 ...
 */
export async function reinitWaaP(providerHint?: SocialProvider): Promise<void> {
  console.log('[.SKI] forcing WaaP re-registration', providerHint ? `(social=${providerHint})` : '');
  ...
  waapWallet = null;
  registered = false;
  await registerWaaP(providerHint);
}
...
export async function registerWaaP(providerHint?: SocialProvider): Promise<void> {
  if (registered || typeof window === 'undefined') return;
  registered = true;
  console.log('[.SKI] WaaP registration starting…', providerHint ? `social=${providerHint}` : '');
  ...
  try {
    // If the caller pre-picked a social provider, restrict the WaaP modal
    // to only that one option so the user lands on a single-button screen.
    const authMethods: AuthenticationMethod[] = providerHint
      ? ['social']
      : ['social', 'email', 'phone'];
    const socials: SocialProvider[] = providerHint
      ? [providerHint]
      : ['twitter', 'google', 'discord', 'github'];
